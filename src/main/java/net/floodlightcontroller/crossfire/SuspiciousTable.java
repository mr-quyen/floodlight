...
public class SuspiciousTable {
	...
	public void locate(Map<IPv4AddressWithMask, Double> updateValues, List<IPv4AddressWithMask> splitted,
		...
		// TODO: from updateValues, by running the algorithm, splitted, merged and blocked will be updated.
		// TODO: updateValues --> elect() --> candidates
        // TODO: some candidates will be promoted to blocked candidates, some will be mergeable candidates...
        //

		//
		update(updateValues); // From this information, blockNodes and suspiciousNodes will be updated.
		
		List<SuspiciousNode> candidates = elect();  // candidates for what??
		...
		logger.info("Experiment - Control - Splitted And Merged Nodes: {}", String.format("![%d] ![%d] ![%d] ![%d]",
				suspiciousNodes.size(), splitted.size(), merged.size(), System.currentTimeMillis()));
	}
	...
	private List<SuspiciousNode> elect() {
		...
		if (root.getAddition() > 0) {
			peerNodes.add(root);
			sum = root.getSuspicious(); //sum is updated here
		}

		List<SuspiciousNode> candidates = new ArrayList<SuspiciousNode>();
		while (true) {
			// at the beginning,
			// peerNodes contains root only
			...
			// sorted peerNodes and parentNodes.
			// they are sorted basde on traffic matched.
			...
			// For Debug
			logger.info("Debug - Suspicious Value - Iteration: {}", String.format("![%d] ![%f]", level, sum));
			for (SuspiciousNode node : peerNodes)
				logger.info("Debug - Suspicious Value - Sorted Peer Nodes: {}", String.format("![%s] ![%f] ![%f]",
						node.getAddress().toString(), node.getSuspicious(), node.getComplementSuspicious()));
			for (SuspiciousNode node : parentNodes)
				logger.info("Debug - Suspicious Value - Sorted Parent Nodes: {}", String.format("![%s] ![%f] ![%f]",
						node.getAddress().toString(), node.getSuspicious(), node.getComplementSuspicious()));

			// start with largest values of peerNodes and parentNodes
			int i = peerNodes.size() - 1, j = parentNodes.size() - 1;
			while (i >= 0 || j >= 0) {
				// first round, no break
				if (aggregation >= sum)
					break;
				...
				if (i < 0) {
					...
					node = peerNodes.get(i); //START: node is taken from here
					aggregation += node.getSuspicious();
					i --;
				} else {
					node = parentNodes.get(j);
				...
				if (node.getChildren().size() == 0) {
					candidates.add(node);
					logger.info("Debug - Suspicious Value - Split Candidate: {}", String.format("![%s] ![%f]",
							node.getAddress().toString(), node.getSuspicious()));
					continue;
				}
				...
				if (node.getPrefix() < level) {
					candidates.add(node);
					logger.info("Debug - Suspicious Value - Split Candidate: {}", String.format("![%s] ![%f]",
							node.getAddress().toString(), node.getComplementSuspicious()));
					continue;
				}
...
//					localSum += child.getSuspicious();
				...
				if (node.getAddition() > 0) {
					...
					// one is peerNodes and one is parentNodes.
					// peerNodes' length is longer than parentNodes
		...
		for (SuspiciousNode candidate : candidates) {
			if (candidate.getPrefix() == maxSplitLevel) {
				...
				if (candidate.incrScore() >= score)
					...
					logger.info("Experiment - Detection - Add Score: {}", String.format("![%s] ![%d] ![%d]",
							candidate.getAddress().toString(), candidate.getScore(), currentTime));
			} else {
		...
		while (splitCandidates.size() != 0) {
			...
			if (suspiciousNodes.size() + demand <= maxLocationRules) {
				...
				logger.info("Experiment - Detection - Split: {}",
						String.format("![%s]", candidate.getAddress().toString()));
			} else {
				splitCandidates.add(candidate);
				break;
			}
...
//					parent.getAddress().toString(), parent.getSuspicious(), parent.getPromotionTime()));
			...
			if (premergeCandidates.size() == demand) {
				...
				for (SuspiciousNode mergeCandidate : premergeCandidates) {
					...
					logger.info("Experiment - Control - Merge: {}",
							String.format("![%s]", mergeCandidate.getAddress().toString()));
				}
				
				splittedNodes.addAll(splitNode(splitCandidate, currentTime));
				logger.info("Experiment - Control - Split: {}",
						String.format("![%s]", splitCandidate.getAddress().toString()));
			} else 
				break;
		}
	}
	...
	protected class SuspiciousNode{
		...
		// number of bit used for prefix
        private int prefix;

        // One parent
		private SuspiciousNode       parent;
        // And list of children (4 here )
		private List<SuspiciousNode> children;
		
		// refer to Jing's email to understand these fields
        private double cumulation;
		private double complement;
		private double addition;

        // to decide to block a node or not
		private int    score;
