...
//				IOFSwitchListener, IFloodlightModule, IRadarServiceHW {
//
//	// Services
...
//
//	// TODO: Dedicated for Crossfire and hardware environment.
//	protected Map<OFSwitchPort, U64> switchPortVectors;
//
...
//
//	protected int  packetLength = 1000;
//	protected long minInterval  = 2000;
//
//	// Logger
...
//		}
//
//		public void setReplies(List<OFStatsReply> replies) {
...
//
//		// TODO: Dedicated for Crossfire and hardware environment.
//		OFSwitchPort swPort = new OFSwitchPort(dpid, Integer.parseInt(vector.getMatchField(MatchUtils.STR_IN_PORT)));
//		switchPortVectors.put(swPort, vectorId);
//
//		if (!switchDetectionVectors.containsKey(dpid))
//			switchDetectionVectors.put(dpid, new ArrayList<U64>());
...
//
//		List<IPv4AddressWithMask> rootDetectionRule = new ArrayList<IPv4AddressWithMask>();
//		rootDetectionRule.add(IPv4AddressWithMask.of("0.0.0.0/0"));
//		addDetectionRules(vectorId, dpid, rootDetectionRule);
//
//		return vectorId;
//	}
//
...
//			// dispatch means: dispatch the results to other modules
//			while(true) {
...
//					/// QUYEN__
//					/// RadarManager and Crossfire communicate together here.
...
//
//			case COL_FLOW_STATS:
//				for (OFStatsReply reply : task.getReplies()) {
//				    OFFlowStatsReply value = (OFFlowStatsReply)reply;
//
//					for (OFFlowStatsEntry e : value.getEntries()) {
//
//						// TODO: For Crossfire attack. For hardware environment.
//						if (e.getMatch().get(MatchField.IN_PORT) == null)
//							continue;
//
//						// TOOD: For Crossfire attack. For hardware environment.
//						OFSwitchPort swPort = new OFSwitchPort(dpid, e.getMatch().get(MatchField.IN_PORT).getPortNumber());
//						U64    vId     = switchPortVectors.get(swPort);
//
//						long   packets = e.getPacketCount().getValue();
//						long   bytes   = packets * packetLength;
...
//
//							logger.debug("Debug - Statistic - Detection Flow Statistics: {}",
//									String.format("![%s] ![%s] ![%s] ![%s/%d] ![%d] ![%d] ![%f] ![%d]",
//											key, swPort.toString(), vId.toString(), address.toString(), prefix,
//											packets, bytes,	speed, now));
//						}
//					}
//				}
//				break;
...
//
//			if (splitted.size() != 0) {
//				addDetectionRules(vectorId, dpid, splitted);
//				long now = System.currentTimeMillis();
...
////	}
//
//	private void addDetectionRules(U64 vectorId, DatapathId dpid, List<IPv4AddressWithMask> addresses) {
//		DetectionVectorDescriptor vector = detectionVectors.get(vectorId);
//		List<Task> tasks = new ArrayList<Task>();
//
//		IOFSwitch sw = switchService.getSwitch(dpid);
//
//		// Prepare forwarding action
////		OFAction forwarding = sw.getOFFactory().actions().buildOutput()
////				.setPort(OFPort.of(1)).build();
////		List<OFAction> actions = new ArrayList<OFAction>();
////		actions.add(forwarding);
//
...
//
//	private class Trace {
//		private Map<DatapathId, Boolean> activated;
//		private Map<DatapathId, Boolean> inProgress;
//		private Map<DatapathId, Long>    latestTime;
//
//		public Trace() {
//			activated  = new HashMap<DatapathId, Boolean>();
...
//				return activated.get(dpid);
//			return false;
//		}
//
//		public Map<DatapathId, Boolean> getActivatedDetection() {
//			return activated;
//		}
//
...
//		}
//
//		public void setActivated(DatapathId dpid, boolean isActivated) {
//			activated.put(dpid, isActivated);
//		}
//
...
//
//		trace.setActivated(switchId, true);
//		trace.setLatestTime(switchId, System.currentTimeMillis());
//
//		if (!switchDetectionVectors.containsKey(switchId))
...
//
//		logger.info("Experiment - Parameter - Packet Length: {}", String.format("![%d]", packetLength));
//		logger.info("Experiment - Parameter - Minimum Interval: {}", String.format("![%d]", minInterval));
//
//
//		// Initialize basic classes
//		trace     = new Trace();
//		collector = new Collector();  // Collect statistics (link, flow stats).
//		locator   = new Locator();    // Locate the anomalous flows.
//		scheduler = new Scheduler(numWorker); // depend on machine capacity
//
...
//		switchPortVectors = new HashMap<OFSwitchPort, U64>();
//	}
//
//	// Module startup
//	@Override
...
//	    m.put(IRadarServiceHW.class, this);
//	    return m;
//	}
//
//	@Override
...
//
//		String sPacketLength = configParams.get("packetLength");
//        if (!Strings.isNullOrEmpty(sPacketLength)) {
//            try {
//            	packetLength = Integer.parseInt(sPacketLength);
//            } catch (NumberFormatException e) {
//                logger.error("Error - Setting - Invalid packet length specifier:", e);
//            }
//            logger.info("Experiment - Setting - Packet Length: {}", String.format("![%d]", packetLength));
//        }
//
//        String sMinInterval = configParams.get("minInterval");
//        if (!Strings.isNullOrEmpty(sMinInterval)) {
//            try {
//            	minInterval = Integer.parseInt(sMinInterval);
//            } catch (NumberFormatException e) {
//                logger.error("Error - Setting - Invalid minimum interval specifier:", e);
//            }
//            logger.info("Experiment - Setting - Minimum Interval: {}", String.format("![%d]", minInterval));
//        }
//	}
//}
