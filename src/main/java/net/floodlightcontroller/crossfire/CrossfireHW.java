...
public class CrossfireHW implements IFloodlightModule, IOFSwitchListener, ILinkDiscoveryListener, IRadarListener {
	...
	protected int maxFlowEntries = 5000;
	...
	protected IRadarServiceHW              radarService;
	
	private Thread updater;
	private BlockingQueue<UpdateEvent> updateQueue;
	
	private U64 cookie;  // cookie is used to distinguish flows 
	...
	protected Map<OFSwitchPort, PortStatistic>                           historyPortStats;  // not link statistics? 
	...
	protected void updateThread() {
		/*
		 * 
		 * This thread does all main works: 
		 * periodically queries switches and parses data to get port statistics and flow statistics. 
		 * 
		 */
		UpdateEvent update;
		while (true) {
			...
			// When there is an update event raised, 
			// event is parsed to get statistics
			// Since the update may come from flow or port statistics - 
			// (i): PORT_STAT update: put the port data to the dictionary
			// (ii): FLOW_STAT update
			
			DatapathId dpid = update.getDatapathId();
			Map<String, OFStatistic> stats = update.getStatistic();
			
			Map<OFSwitchPort, PortStatistic> portStats = new HashMap<OFSwitchPort, PortStatistic>();  // port statistics 
			...
			for (OFSwitchPort swPort : portStats.keySet()) {
				...
				if (portStat.getSpeed() >= thWarning * capacity && !criticalPortWaves.containsKey(swPort)) {
					logger.info("Experiment - Detection - Initialize Location Vector: {}",
...
//								dpid, CollectionMode.COLLECTION_MODE_ACTIVATED);
				...
				if (portStat.getSpeed() <= thWarning * capacity || portStat.getSpeed() >= thCongestion * capacity) {
					historyPortStats.put(swPort, portStat); //Quyen: finally, the updates are put into global maps 
	...
	private void detectSuspicious(OFSwitchPort swPort, PortStatistic portStat,
		...
		if (splitted.size() != 0 || merged.size() != 0 || blocked.size() != 0) {
			...
			for (IPv4AddressWithMask m : merged) {
				historyFlowStats.get(swPort.getDatapathId()).remove(m);
				historyFlowStats.get(swPort.getDatapathId()).remove(m);
			}
	...
	protected class OFLink {
		...
		private String key; // key to identify links 
						 // The constructor will set the key, using provided inputs
						 // normally, switch link identified by two ports, through which switches are connected
	...
	public void init(FloodlightModuleContext context)
		...
		radarService         = context.getServiceImpl(IRadarServiceHW.class);
	...
	public void startUp(FloodlightModuleContext context)
		...
		updater = new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					updateThread();  // always run? no need timer? 
				}
			}
		}, "Updater");
		updater.start();
